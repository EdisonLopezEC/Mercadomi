    ...
    <section v-if="products.length > 0" class="mb-7 sm:mb-12">
        ...
        <div class="container">
            <div class="flex items-center justify-between gap-4 mb-5 sm:mb-7">
                <h2
                    class="text-2xl sm:text-4xl font-bold capitalize text-yellow"
                >
                    ALIMENTOS
                </h2>
                <router-link
                    :to="{ path: '/product', query: { category: 'ALIMENTOS' } }"
            ...
            <div>
                <div class="block w-full h-full relative">
                    ...
                    <router-link
                        :to="{
                            path: '/product',
                            query: { category: 'frutas-y-verduras' },
                        }"
                        class="banner-button"
                    >
                        Mostrar más
        ...
        <div class="container">
            <div class="flex items-center justify-between gap-4 mb-5 sm:mb-7">
                ...
                <router-link
                    :to="{ path: '/product', query: { category: 'lacteos' } }"
                    class="py-2 px-4 text-sm sm:py-3 sm:px-6 rounded-3xl capitalize sm:text-base font-semibold whitespace-nowrap bg-primary-slate text-primary transition-all duration-300 hover:bg-primary hover:text-white"
                >
                    Mostrar más
                </router-link>
            ...
            <div>
                <div class="block w-full h-full relative">
                    <img
                        ...
                        loading="lazy"
                    />
                    <a href="/#/supermercado" class="banner-button"
                        >Mostrar mas</a
                    >
                </div>
            </div>
...
export default {
    ...
    computed: {
        products: function () {
            return this.$store.getters["frontendProduct/popularProducts"].slice(
                0,
                4
            );
        },
    ...
    mounted() {
        ...
        this.$store
            .dispatch("frontendProduct/popularProducts", {
                paginate: 0,
                rand: 8,
            })
            .then((res) => {
